...
```{r}
jpeg("../Figures/Fig1_Old.jpg", width = 8, height = 5, units="in", res=600)
fig1
```
...
ak_map <- ggplot(data = world) +
  ...
  geom_rect(aes(xmin = -138, xmax = -133, ymin = 56, ymax = 59.5), fill = "transparent", color = "black") 
...
seak_v2 <- ggplot() +
  geom_sf(data = map_gshhs) + 
  coord_sf(xlim = c(-138, -133), ylim = c(56,59.5)) + 
  theme_pubr() +
  geom_point(data = bc, aes(x = longitude, y = latitude, fill = Location), size = 3, 
        shape = 21) +
  scale_fill_manual(values = c("black")) +
...
#revised manuscript map

#seak map

```{r}
seak_area_updated <- ggplot() +
  geom_sf(data = map_gshhs) + 
  coord_sf(xlim = c(-138, -132), ylim = c(56,59.5)) + 
  scale_x_continuous(breaks = c(-132, -134, -136, -138)) +
  scale_y_continuous(breaks = c(56, 57, 58, 59)) + 
  theme_pubr() +
  annotation_scale(location = "bl", width_hint = 0.4) +
  annotation_north_arrow(location = "bl", which_north = "true", 
        pad_x = unit(0.0, "in"), pad_y = unit(0.2, "in"),
...
```{r}
ak_map_updated <- ggplot(data = world) +
  geom_sf() +
  coord_sf(xlim = c(-179.8, -115),
           ylim = c(72, 50),
           expand = FALSE) +
  ...
  theme(legend.position = "none",
   ...
   theme(panel.background = element_rect(fill = "white"),
        panel.ontop = FALSE) +
  geom_rect(aes(xmin = -138, xmax = -132, ymin = 56, ymax = 59.5), fill = "transparent", color = "black", size = 1) 
ak_map_updated
```
...
```{r}
map_updated <- ggdraw(seak_area_updated) +
  draw_plot(
    {ak_map_updated
      },
    # The distance along a (0,1) x-axis to draw the left edge of the plot
