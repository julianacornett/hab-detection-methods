...
```{r}
count_sub <- select(count_micro, c("Date", "Count_1m_L", "Count_5m_L"))
```

#gather into long form

```{r}
count_long <- melt(count_sub, id.vars = "Date", variable.name = "Method")
```

#select only ID_yyyymmdd & Cells_L from eDNA sample-only dataframe

```{r}
qpcr_sub <- select(Quant_dailyMean, c("Date", "Quantity_Mean"))
```

#add "Method" column filled with "qPCR"

```{r}
qpcr_sub$Method <- "qPCR"
```

#rename Cells_L to "value" to match

```{r}
qpcr_sub <- qpcr_sub %>% rename("value" = "Quantity_Mean")
```

```{r}
qpcr_sub$value <- as.integer(qpcr_sub$value)
```

#append to long-form microscope count df

```{r}
count_qpcr <- rbind(count_long, qpcr_sub)
```
...
#need to find way to have 2 y-axes
