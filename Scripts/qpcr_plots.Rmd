...
Quant_dailyMean$Cells_L <- Quant_dailyMean$Copies_L_Corrected/11107
Quant_dailyMean
max(Quant_dailyMean$Cells_L)
min(Quant_dailyMean$Cells_L)
mean(Quant_dailyMean$Cells_L)
```
...
```{r}
lm_net_qpcr <- lm(count_qpcr$Count_Net ~ count_qpcr$log_qpcr)
summary(lm_net_qpcr)
```
...
```{r}
count_depths$Depth <- gsub("1m", "1 m", count_depths$Depth)
count_depths$Depth <- gsub("5m", "5 m", count_depths$Depth)

qpcr_mic_corr <- ggplot(count_depths, aes(x = log_qpcr, y = Count_L, color = Depth, fill = Depth, shape = Depth)) + 
  scale_fill_manual(values = c("darkgray", "black"), labels = c("1 m", "5 m")) +
  scale_color_manual(values = c("darkgray", "black")) +
  scale_shape_manual(values = c(21,24)) +
  geom_point(size = 4, color = "black", alpha = 0.75) + 
  stat_poly_line(se = FALSE, size = 2) +
  #stat_poly_eq(use_label(c("R2", "p"))) +
  geom_text(aes(x = 15, y = 1350, label = "r^2 == 0.38~~~p == 0.01"), parse = TRUE, color = "darkgray", check_overlap = TRUE) +
  geom_text(aes(x = 15, y = 1250, label = "r^2 == 0.29~~~p == 0.03"), parse = TRUE, color = "black", check_overlap = TRUE) +
  theme_pubr() + 
  labs(
...
lm_1m_qpcr <- lm(lm_1m_qpcr$Count_L ~ lm_1m_qpcr$log_qpcr)
summary(lm_1m_qpcr)
```
...
lm_5m_qpcr <- lm(lm_5m_qpcr$Count_L ~ lm_5m_qpcr$log_qpcr)
summary(lm_5m_qpcr)
```
