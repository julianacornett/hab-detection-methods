...
```{r}
lm_elisa_mba <- lm(pst_pairs$PST_ELISA_ug100g ~ pst_pairs$PST_DEC_ug100g)
...
```{r}
merged_long$log_qpcr <- log(merged_long$qPCR_qmeandaily)
```
...
corr_1m_tox <- ggplot(merged_long, aes(x = Count_1m_L, y = Tox_ug_100g, color = Method, fill = Method, shape = Method)) + 
  ...
  stat_poly_eq(use_label(c("R2", "p"))) +
  theme_pubr() +
  xlab("Alexandrium Count (cells/L)") +
  ylab("PSTs (ug/100g)") +
  theme(legend.position = "right") +
  geom_hline(yintercept = 80, linetype = "dashed") +
  geom_hline(yintercept = 34, linetype = "dotted") +
  annotate("text", x=1400, y=250, label = "A", size = 5) 
corr_1m_tox
```
...
```{r}
jpeg("../Figures/Mic_Tox_Corr.jpg", width = 8, height = 10, units="in", res=600)
corr_1m_tox /
  corr_5m_tox
```
