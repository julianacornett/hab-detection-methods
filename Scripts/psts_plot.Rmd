...
```{r}
merged_long <- read.csv("../Data/PST_Pairs_Long.csv")
merged_long
```
...
```{r}
  qpcr_tox <- ggplot(merged_long, aes(x = qPCR_qmeandaily, y = Tox_ug_100g, color = Method, fill = Method, shape = Method)) + 
...
```{r}
jpeg("../Figures/qPCR_Tox_Correlation.jpg", width = 8, height = 5, units="in", res=600)
qpcr_tox
```

#correlation between microscope counts and psts
