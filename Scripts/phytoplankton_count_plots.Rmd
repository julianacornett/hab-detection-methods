...
```{r}
palette_alex <- hcl.colors(10, palette = "Dynamic")
```

```{r}
"#DB9D85" "#C2A968" "#9DB469" "#6DBC86" "#3DBEAB" "#4CB9CC" "#87AEDF" "#BB9FE0" "#DA95CC" "#E494AB"
```
...
plot_net <- ggplot(alex, aes(x = Date, y = Count_Net)) +
  geom_point(size = 5, shape = 21, fill = "#9DB469") +
  theme_pubr() +
...
plot_1m <- ggplot(alex, aes(x = Date, y = Count_1m_L)) +
  geom_point(size = 5, shape = 21, fill = "#3DBEAB") +
  theme_pubr() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b %Y") +
...
plot_5m <- ggplot(alex, aes(x = Date, y = Count_5m_L)) +
  geom_point(size = 5, shape = 21, fill = "#87AEDF") +
  theme_pubr() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b %Y") +
