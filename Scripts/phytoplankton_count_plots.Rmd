...
#try quadratic since non-significant

```{r}
quad_1m_5m <- lm(alex_na$Count_1m_L ~ poly(alex_na$Count_5m_L,2))
summary(quad_1m_5m)
```
...
```{r}
log_1m_5m <- lm(alex_na$Count_1m_L ~ log(alex_na$Count_5m_L))
summary(log_1m_5m)
```
...
```{r}
exp_1m_5m <- lm(log(alex_na$Count_1m_L) ~ alex_na$Count_5m_L)
summary(exp_1m_5m)
```
...
```{r}
plot_1m_5m <- ggplot(alex_na, aes(Count_1m_L, Count_5m_L)) + geom_point() + geom_smooth(method = "lm", formula = y ~ poly(x,2)) + theme_pubr()
plot_1m_5m
```
