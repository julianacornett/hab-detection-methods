...
jpeg("../New Figures/Fig4.jpg", width = 8, height = 5, units="in", res=600)
pst_ros_time
```
...
jpeg("../New Figures/Fig2.jpg", width = 8, height = 5, units="in", res=600)
counts_time_final
```
...
qpcr_time <- ggplot(qpcr_mean, aes(x = Date, y = log_qpcr)) + 
  geom_point(size = 4, color = "black", alpha = 0.75) + 
  theme_pubr() + 
  labs(x = element_blank(), y = "*A. catenella* qPCR Quantity (Copies/L)") + theme(axis.title.y = ggtext::element_markdown())
 qpcr_time
```
...
```{r}
jpeg("../New Figures/Fig3.jpg", width = 8, height = 5, units="in", res=600)
qpcr_time
```
