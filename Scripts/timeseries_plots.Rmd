...
counts_time <- ggplot(alex, aes(x=Date)) +
  ...
  scale_y_continuous(
    name = "*A. catenella* count at 1 m or 5 m (cells/L)",
...
alex_long$Method[alex_long$Method=="Count_1m_L"] <- "1 m"
alex_long$Method[alex_long$Method=="Count_5m_L"] <- "5 m"
...
qpcr_time <- ggplot(qpcr_mean, aes(x = Date, y = Copies_L_Corrected)) + 
  geom_point(size = 4, color = "black", alpha = 0.75) + 
  theme_pubr() + 
  labs(x = element_blank(), y = "Mean *A. catenella* qPCR quantity (copies/L)") + theme(axis.title.y = ggtext::element_markdown()) +
