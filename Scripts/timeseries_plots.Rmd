...
counts_time <- ggplot(alex, aes(x=Date)) +
  ...
  geom_point(aes(y=Count_Net), fill = "white", shape = 22, size = 3, alpha = 0.75) + 
  ...
  labs(x = element_blank(), y = "*A. catenella* Count (Cells for Net; Cells/L for 1m & 5m)") + 
  theme(axis.title.y = ggtext::element_markdown(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) 
...
qpcr_time <- ggplot(qpcr_mean, aes(x = Date, y = Copies_L_Corrected)) + 
  ...
  labs(x = element_blank(), y = "*A. catenella* Quantity (Copy Number/L; Log-transformed)") +
